...
task_name = f"{config['env']}-efs-sync-task"
container_name = f"{config['env']}-efs-sync"
...
with DAG(
    ...
    def configure_dag(**kwargs):
        ...
        push_log_variables(ti,task_definition_name=task_name,container_name=container_name,prefix='efs-sync-task')
    ...
    load_dataset_ecs_task = EcsRunTaskOperator(
        task_id=f"manual-efs-sync-dataset",
        dag=dag,
