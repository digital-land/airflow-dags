import os
import json
import boto3

from datetime import datetime, timedelta
from airflow import DAG
from airflow.decorators import dag
from airflow.providers.amazon.aws.operators.ecs import (
    EcsRegisterTaskDefinitionOperator,
    EcsRunTaskOperator,
)
from airflow.operators.python import PythonOperator
from airflow.models.param import Param

from utils import get_config, get_task_log_config

# read config from file and environment
my_dir = os.path.dirname(os.path.abspath(__file__))
configuration_file_path = os.path.join(my_dir, "config.json")
config  = get_config(configuration_file_path) 

# set some variablles needed for ECS tasks, 
# TODO get from environment in the future
ecs_cluster = f"{config['env']}-cluster"
package_task_defn= f"{config['env']}-mwaa-data-package-builder-task"
aws_vpc_config={
    "subnets": ["subnet-05a0d548ea8d901ab", "subnet-07252405b5369afd3"],
    "securityGroups": ["sg-0fe390dd951829c75"],
    "assignPublicIp": "ENABLED",
}

# set some default_arrgs for all colllections
default_args = {
    "owner": "airflow",
    "depends_on_past": False,
    "start_date": datetime(2024, 1, 1),
    "dagrun_timeout": timedelta(minutes=5),
}

# set task id for the initialisation task at the start
configure_dag_task_id = "configure-dag"

def configure_dag(**kwargs):
    """
    function which returns the relevant configuration details
    and stores them in xcoms for other tasks. this includes:
    - get and process params into correct formats
    - read in env variables
    - access options defined in the task definitions
    """
    # retrieve  and process parameters
    params = kwargs['params']

    memory = int(params.get('memory'))
    cpu = int(params.get('cpu'))
    # transformed_jobs = str(kwargs['params'].get('transformed-jobs'))
    # dataset_jobs = str(kwargs['params'].get('dataset-jobs'))
    

    # get ecs-task logging configuration
    ecs_client = boto3.client('ecs')
    package_task_log_config = get_task_log_config(ecs_client,package_task_defn)
    package_task_log_config_options = package_task_log_config['options']
    package_task_log_group = str(package_task_log_config_options.get('awslogs-group'))
    # add container name to prefix
    package_task_log_stream_prefix = str(package_task_log_config_options.get('awslogs-stream-prefix')) + f'/{package_task_defn}'
    package_task_log_region = str(package_task_log_config_options.get('awslogs-region'))


    # Push values to XCom
    ti = kwargs['ti']
    ti.xcom_push(key='memory', value=memory)
    ti.xcom_push(key='cpu', value=cpu)
    ti.xcom_push(key='package-task-log-group', value=package_task_log_group)
    ti.xcom_push(key='package-task-log-stream-prefix',value=package_task_log_stream_prefix)
    ti.xcom_push(key='package-task-log-region',value=package_task_log_region)


for package in config['packages']:

    with DAG(
        f"{package}-package",
        default_args=default_args,
        description=f"build the {package} package",
        schedule=None,
        params={
            "cpu": Param(default=8192, type="integer"),
            "memory": Param(default=32768, type="integer"),
        },
        render_template_as_native_obj=True,
        tags = ['data-package']
    ) as dag:
        configure_dag_task = PythonOperator(
            task_id=configure_dag_task_id,
            python_callable=configure_dag,
            dag=dag,
        )

        package_ecs_task = EcsRunTaskOperator(
            task_id=f"{package}-package-builder",
            dag=dag,
            execution_timeout=timedelta(minutes=600),
            cluster=ecs_cluster,
            task_definition=package_task_defn,
            launch_type="FARGATE",
            overrides={
                "containerOverrides": [
                    {
                        "name": package_task_defn,
                        'cpu': '{{ task_instance.xcom_pull(task_ids="configure-dag", key="cpu") | int }}', 
                        'memory': '{{ task_instance.xcom_pull(task_ids="configure-dag", key="memory") | int }}', 
                        "environment": [
                            {"name": "DATA_PACKAGE_NAME", "value": package},
                        ],
                    },
                ]
            },
            network_configuration={
                "awsvpcConfiguration": aws_vpc_config
            },
            awslogs_group='{{ task_instance.xcom_pull(task_ids="configure-dag", key="package-task-log-group") }}',
            awslogs_region='{{ task_instance.xcom_pull(task_ids="configure-dag", key="package-task-log-region") }}',
            awslogs_stream_prefix='{{ task_instance.xcom_pull(task_ids="configure-dag", key="package-task-log-stream-prefix") }}',
            awslogs_fetch_interval=timedelta(seconds=1)
        )

        configure_dag_task >> package_ecs_task