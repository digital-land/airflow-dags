...
def get_task_log_config(ecs_client, task_definition_family):
    ...
    log_config = response["taskDefinition"]["containerDefinitions"][0].get(
        "logConfiguration", {}
    )
...
def setup_configure_dag_callable(config, task_definition_name):
    def configure_dag(**kwargs):
        ...
        aws_vpc_config = {
            "subnets": kwargs["conf"]
            .get(section="custom", key="ecs_task_subnets")
            .split(","),
            "securityGroups": kwargs["conf"]
            .get(section="custom", key="ecs_task_security_groups")
            .split(","),
        ...
        collection_task_log_config = get_task_log_config(
            ecs_client, task_definition_name
        )
        collection_task_log_config_options = collection_task_log_config["options"]
        collection_task_log_group = str(
            collection_task_log_config_options.get("awslogs-group")
        )
        # add container name to prefix
        collection_task_log_stream_prefix = (
            str(collection_task_log_config_options.get("awslogs-stream-prefix"))
            + f"/{task_definition_name}"
        )
        collection_task_log_region = str(
            collection_task_log_config_options.get("awslogs-region")
        )
        ...
        ti.xcom_push(
            key="collection-task-log-stream-prefix",
            value=collection_task_log_stream_prefix,
        )
